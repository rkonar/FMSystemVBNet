Public Class clsReport
    ...
    Public Sub genEODReport()
        ...
        ReportSql = "select * from tbljournal where Status is null and ((AccountNo like 'CASH%') or (AccountNo like 'BANK%') or (AccountNo like 'CHEQUE%')) and TxnDate='" & DateTime.Now.ToString("yyyyMMdd") & "' order by AccountNo, DrAmount,CrAmount"
...
errH:
    ...
    Public Sub genDefaulterReport(cutoffAmt As Double, BGFOrAssociation As String)
        ...
        If BGFOrAssociation = "BGF" Then
            ReportSql = "SELECT sum(DrAmount-CrAmount) as Outstanding,AccountNo FROM tbljournal where Status is null and AccountNo like 'DEBTOR-CORPUS-FUND-%' group by AccountNo having Outstanding > " & cutoffAmt
            reportTitle = "BGF Maintenance and Lift AMC Dues Defaulter List** "
        ElseIf BGFOrAssociation = "ASSOCIATION" Then
            ReportSql = "SELECT sum(DrAmount-CrAmount) as Outstanding,AccountNo FROM tbljournal where Status is null and AccountNo like 'DEBTOR-FLAT-%' group by AccountNo having Outstanding > " & cutoffAmt
        ...
        While actRS1.EOF = False
            ...
            ' ''ReportSql = "SELECT sum(DrAmount-CrAmount) as BGFOutstanding FROM tbljournal where AccountNo = 'DEBTOR-CORPUS-FUND-" & flatCode & "'"
...
errH:
    ...
    Sub genCASHBANKBOOKReport(accountNo As String, includeSummary As Boolean)
        ...
        ssql = "select sum(DrAmount+0) as netDrAmt, sum(CrAmount+0) as netCrAmt from tbljournal where Status is null and AccountNo ='" & accountNo & "'"
        ...
        'print rest
        ReportSql = "select * from tbljournal where Status is null and AccountNo ='" & accountNo & "'"
...
errH:
    ...
    End Sub
    Sub genExpenseReport(includeSummary As Boolean)
        On Error GoTo errH
        ...
        'ReportSql = "select * from tbljournal where Status is null and TxnType in ('EXPENSE','PREPAID-EXPENSE')"
        'ReportSql = ReportSql & " and (AccountNo ='CASHBOX' OR AccountNo ='DD' OR AccountNo LIKE 'CHEQUE%' OR AccountNo LIKE 'BANK%')"  
        ReportSql = "select * from tbljournal where Status is null "
        ...
        While actRS1.EOF = False
            ...
            If onlySummary = False Then
                ...
                templateSht.Cells(r, "A").value = actRS1("AccountNo").Value.ToString
                templateSht.Cells(r, "B").value = getDrORCrAccountNo(actRS1("gid").Value.ToString, "CR", "tbljournal")
                templateSht.Cells(r, "C").value = formatInt2Date(actRS1("TxnDate").Value.ToString)
                templateSht.Cells(r, "D").value = actRS1("TxnType").Value.ToString
                templateSht.Cells(r, "E").value = actRS1("Narration").Value.ToString
                templateSht.Cells(r, "F").value = actRS1("DocRef").Value.ToString
