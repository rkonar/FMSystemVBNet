Public Class clsReceipt
    ...
    Public Sub loadSingleReceipt(receipt_id As String, receipt_no As String, g_id As String)
        ...
        If receipt_id <> "" Then
            ssql = "select * from tblreceipt where tblid=" & receipt_id
        ElseIf receipt_no <> "" Then
            ssql = "select * from tblreceipt where ReceiptNo='" & receipt_no & "'"
        ElseIf g_id <> "" Then
            ssql = "select * from tblreceipt where gid='" & g_id & "'"
        Else
            resetAll()
            MsgBox("clsReceipt/loadSingleReceipt: atleast one of the input should be non blank")
            Exit Sub
        End If
        ...
        If receiptRS.EOF = True Then
            Status = False
            MsgBox("clsReceipt/loadSingleReceipt: No receipt found for tblid/receiptno=" & receipt_id & "/" & receipt_no)
            Exit Sub
        End If
        ...
        ssql = "select TxnDate from tbljournal where gid='" & gid & "'"
        receiptRS = gcon.Execute(ssql)
...
errH:
    ...
    Public Sub CreateReceipt()
        ...
        ssql = "select tblid from tblreceiptbook where ReceiptNo = '" & ReceiptNo & "' and Status='A'"
        ...
        ssql = "Insert into tblreceipt (gid,refgid,ReceiptNo,FlatCode,ReceiptDate,ReceiptAmt,InstrumentType,InstrumentDate,InstrumentBank,InstrumentNo,Remarks) values ("
        ssql = ssql & "'" & gid & "',"
        If refgid = "" Then
            ssql = ssql & "null,"
        Else
        ...
        ssql = "Update tblreceiptbook set Status='U' where ReceiptNo='" & ReceiptNo & "'"
        gcon.Execute(ssql)
...
errH:
    ...
    Public Sub UpdateReceipt()
        ...
        ssql = "Update tblreceipt set " '(ReceiptNo,FlatCode,ReceiptDate,ReceiptAmt,InstrumentType,InstrumentDate,InstrumentBank,InstrumentNo,Remarks) values "
        ...
        Dim thisJournal As New clsJournal, tmpDocRef As String = ""
        ...
        While journalRS.EOF = False
            ...
            If InstrumentType = "CASH" Then
                ...
                thisJournal.DrAccountNo = "CASHBOX"
            ...
            ElseIf InstrumentType = "CHEQUE" Then
                ...
                thisJournal.DrAccountNo = "CHEQUE-IN-HAND"
                'thisJournal.TxnDate = ReceiptDate
        ...
        ssql = "update tbljournal set Narration='" & tmpDocRef & "' where refgid='" & gid & "'"
        gcon.Execute(ssql)
...
errH:
    ...
    Public Sub ContraReceipt()
        ...
        'Update in tblreceipt
        ssql = "Update tblreceipt set Status='C'" ', refgid=null" '(ReceiptNo,FlatCode,ReceiptDate,ReceiptAmt,InstrumentType,InstrumentDate,InstrumentBank,InstrumentNo,Remarks) values "
        ...
        Dim thisJournal As New clsJournal, CrLine As New clsJournal, DrLine As New clsJournal
        ...
        'ssql = "Update tbljournal set Status='C'" ',refgid=null" '(ReceiptNo,FlatCode,ReceiptDate,ReceiptAmt,InstrumentType,InstrumentDate,InstrumentBank,InstrumentNo,Remarks) values "
        'ssql = ssql & ", Narration=concat('[RECEIPT-CANCELLED:" & Replace(Trim(Remarks), "'", "''") & "] ', Narration)"
        'ssql = ssql & " where gid='" & gid & "'"
        'gcon.Execute(ssql)

        ''Unlink reconciled bank txns, if any
        'ssql = "Update tbljournal set refgid=null" '(ReceiptNo,FlatCode,ReceiptDate,ReceiptAmt,InstrumentType,InstrumentDate,InstrumentBank,InstrumentNo,Remarks) values "
        'ssql = ssql & ", Narration=concat('[RECEIPT-CANCELLED:" & Replace(Trim(Remarks), "'", "''") & "] ', Narration)"
...
errH:
    ...
    '        'Update in tblreceipt
    '        ssql = "Update tblreceipt set "  'InstrumentReconciled='Y'" '(ReceiptNo,FlatCode,ReceiptDate,ReceiptAmt,InstrumentType,InstrumentDate,InstrumentBank,InstrumentNo,Remarks) values "
