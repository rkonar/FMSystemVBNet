...
errH:
    ...
    Sub JournalPayment()
        ...
        With thisJournal
            ...
            If (cboPaymentMode.Text <> "ONLINE") And (cboPaymentMode.Text <> "") Then
                ssql = "Update tblvoucherbook set Status='U' where VoucherNo=" & stripLeadingZero(Trim(cboVoucherNo.Text))
                gcon.Execute(ssql)
            End If
...
errH:
    ...
    Sub loadDebitCombo()
        ...
        ElseIf cboExpenseType.Text = "ADJUST-AGAINST-DEFERRED" Then
            ...
            'ssql = "select AccountNo from tblaccounts where Status='A' and LeafAccount='Y' and AccountType='Liabilities' and AccountNo like '" & expAccountType & "%'"
            ssql = "select AccountNo from tblaccounts where Status='A' and LeafAccount='Y' and AccountType='Liabilities' and (ParentAccountNo = 'DEFERRED-LIAB' Or AccountNo in ('PROVISION-FOR-AUDIT-FEE','PROVISION-FOR-EXPENSE','PROVISION-FOR-TAX'))"
    ...
    Sub loadLiabilitiesAccounts()
        ...
        ssql = "select AccountNo from tblaccounts where Status='A' and AccountType='Liabilities' and LeafAccount='Y' and AccountNo like 'DEFERRED-LIAB%'"
        expActRS = gcon.Execute(ssql)
    ...
    Sub loadDeferredExpAccounts()
        ...
        cboCreditAccount.Items.Clear()
        ssql = "select AccountNo from tblaccounts where Status='A' and LeafAccount='Y' and AccountType='Assets' and AccountNo like 'DEFERRED-EXP%'"
        expActRS = gcon.Execute(ssql)
    ...
    Sub loadAvailableVouchers()
        ...
        tmpRS = gcon.Execute("select VoucherNo from tblvoucherbook where Status='A' order by VoucherNo ASC")
    ...
    Sub loadExpenseApprovers()
        ...
        tmpsql = "select u.FullName from tblmap_role_user ru, tblusers u, tblroles r where r.Status='A' and r.tblid=ru.RoleID and ru.UserID=u.tblid and r.RoleCode = 'ExpenseApprovers'"
    ...
    Sub resetTheForm()
        ...
        If gLoginID <> "sa" Then
            ...
            btnEnableVoucherNoUpdate.Visible = False
            btnUpdateVoucherNo.Visible = False
        End If
    ...
    Sub restrictForBankChargeOnly()
        If gAllowBankChargeEntryOnly = True Then
            ...
            cboDebitAccount.Text = "EXPCUR-BANKCHARGES"
    ...
    Function SearchCheque() As Boolean
        ...
        If cboChequeNo.Text = cboInstrumentNo.Text Then
            ssql = "select ReceiptNo,ReceiptAmt from tblreceipt where InstrumentNo='" & cboChequeNo.Text & "'"
            expActRS = gcon.Execute(ssql)
            ...
            If expActRS.EOF = False Then
                ...
                ssql = "select * from tbljournal where AccountNo like'BANK%' and DocRef like '%Cheque No%" & cboChequeNo.Text & "%'"
                expActRS = gcon.Execute(ssql)
                If expActRS.EOF = False Then
    ...
    Sub PopulateJournal_b(onlyPending As Boolean, onlyCurUser As Boolean)
        ...
        With dgvJournal_b
            ...
            .Columns("DrAccountNo_b").Visible = False
        ...
        journalQuerySQL = "select * from tbljournal where CrAmount=0 " 'and status is null"
        ...
        Else
            ...
            ElseIf cboExpenseType.Text = "DEFERRED" Then
                journalQuerySQL = journalQuerySQL & " and TxnType='EXPENSE' and AccountNo like 'DEFERRED-%' "
            ElseIf cboExpenseType.Text = "FIXED-ASSET" Then
                journalQuerySQL = journalQuerySQL & " and TxnType='FIXED-ASSET' "
            ElseIf cboExpenseType.Text = "PREPAID" Then
                journalQuerySQL = journalQuerySQL & " and TxnType='PREPAID-EXPENSE' "
            ElseIf cboExpenseType.Text = "ADJUST-AGAINST-DEFERRED" Then
                journalQuerySQL = journalQuerySQL & " and TxnType='POSTPAID-EXPENSE' "
            Else
        ...
        With dgvJournal_b
            ...
            While rsJournal.EOF = False
                ...
                '=======get credit a/c no=======
                journalQuerySQL2 = "select AccountNo from tbljournal where DrAmount=0 and gid='" & rsJournal("gid").Value.ToString & "'"
...
errH:
    ...
    Private Sub DisplayVoucher()
        ...
        If dgvJournal_b.Rows(ri).Cells("Status").Value = "Pending" Then
            ...
            If isMemberOfRole(gLoginID, "ExpenseApprovers") = True Then
    ...
    Sub eMailApprover(action As String) 'action=NOTIFY-CREATE|NOTIFY-UPDATE|APPROVED|REJECTED
        ...
        With myMailer
            ...
            If action = "NOTIFY-CREATE" Then
                .mailSubject = "Payment Approval Notification : Greenfield Heights(HIG) Association : (Rs. " & txtPaymentAmount.Text & ")"
            ...
            ElseIf action = "NOTIFY-UPDATE" Then
                .mailSubject = "Payment Approval Notification [UPDATE] : Greenfield Heights(HIG) Association : (Rs. " & txtPaymentAmount.Text & ")"
                tmpstr = "Dear " & cboApprovers.Text & "<br>"
            ...
            ElseIf action = "APPROVED" Then
                .mailSubject = "Payment Approved : Greenfield Heights(HIG) Association : (Rs. " & txtPaymentAmount.Text & ")"
                tmpstr = "Dear " & cboApprovers.Text & "<br>"
            ...
            ElseIf action = "REJECTED" Then
                .mailSubject = "Payment Rejected : Greenfield Heights(HIG) Association : (Rs. " & txtPaymentAmount.Text & ")"
                tmpstr = "Dear " & cboApprovers.Text & "<br>"
            ...
            If cboApprovers.Text <> "" Then
                .SendMail_Expense(getUserEmail(cboApprovers.Text))
...
errH:
    ...
    Private Sub cboDebitAccount_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboDebitAccount.SelectedIndexChanged
