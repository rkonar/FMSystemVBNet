Public Class frmBankReconcileCultural
    ...
    Sub PopulateJournal_b() 'FROM BANK
        ...
        '1st part
        'journalQuerySQL = "select * from tbljournal_cult where status is null"
        journalQuerySQL = "select * from tbljournal_cult where 1=1"
...
errH:
    ...
    Sub PopulateJournal_a() 'FROM SYSTEM
        ...
        '1st part
        'journalQuerySQL = "select * from tbljournal_cult where AccountNo <> 'CASHBOX-CULT' and status is null and TxnType <> 'RECEIPT-BGF' "  'Exclude Earth Movers & Builders Pvt. Ltd. Receipts
        journalQuerySQL = "select * from tbljournal_cult where AccountNo <> 'CASHBOX-CULT' " ' and TxnType <> 'RECEIPT-BGF' "  'Exclude Earth Movers & Builders Pvt. Ltd. Receipts
        ...
        If optDeposit.Checked = True Then
            ...
            journalQuerySQL = journalQuerySQL & " and CrAmount=0 and (AccountNo='CHEQUE-IN-HAND-CULT' OR AccountNo='NEFT-NOT-RECONCILED-CULT') "
            If txtFromAmount.Text <> "" Then
                journalQuerySQL = journalQuerySQL & " and ((DrAmount >=" & txtFromAmount.Text & " and CrAmount=0) OR (CrAmount >=" & txtFromAmount.Text & " and DrAmount=0))"
            End If
            If txtToAmount.Text <> "" Then
        ...
        ElseIf optWithdrawal.Checked = True Then
            ...
            journalQuerySQL = journalQuerySQL & " and DrAmount=0 and (AccountNo like 'CHEQUE-ISSUED-CULT-%') "
            journalQuerySQL = journalQuerySQL & " and DocRef like '%Cheque No%'"
            If txtFromAmount.Text <> "" Then
                journalQuerySQL = journalQuerySQL & " and ((CrAmount >=" & txtFromAmount.Text & " and DrAmount=0) OR (DrAmount >=" & txtFromAmount.Text & " and CrAmount=0))"
            End If
...
errH:
    ...
    Private Sub frmBankReconcileCultural_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        'setFormSize()
        ...
        For r_a = dgvJournal_a.Rows.Count - 1 To 0 Step -1
            ...
            For r_b = dgvJournal_b.Rows.Count - 1 To 0 Step -1
                ...
                If (chqNo_a <> "") And (chqNo_a = chqNo_b) And (amt_a = amt_b) Then
                    ...
                    If optDeposit.Checked = True Then
                        ssql = "update tbljournal_cult set refgid='" & gid_a & "', Narration='" & dRef_a & "' where gid='" & gid_b & "'"
                    ElseIf optWithdrawal.Checked = True Then
                        ssql = "update tbljournal_cult set refgid='" & gid_a & "', Narration='" & narration_a & "' where gid='" & gid_b & "'"
                    End If
                    gcon.Execute(ssql)

                    'Link journal with bank
                    ssql = "update tbljournal_cult set refgid='" & gid_b & "' where gid='" & gid_a & "'"
                    gcon.Execute(ssql)
...
errH:
    ...
    Private Sub btnOpenJournalBankStatement_Click(sender As System.Object, e As System.EventArgs) Handles btnOpenJournalBankStatement.Click
        ...
        gItemCode = "frmJournalBankStatementCultural"
        getRolePermission()
        If gCanView <> "Y" Then
            gItemCode = "frmJournalBankStatementCultural"
        ...
        frmJournalBankStatementCultural.ShowDialog()
        frmJournalBankStatementCultural.Dispose()
        gItemCode = "frmBankReconcileCultural"
    End Sub
...
errH:
    ...
    Private Sub PerformManualReconciliation(r_a As Integer, r_b As Integer, aIndx As Integer)
        ...
        '_b=bank; _a=system
        'Link bank with journal
        ssql = "update tbljournal_cult set refgid='" & gid_a & "',"
        ...
        'Link receipt with bank
        ssql = "Update tblreceipt_cult set refgid='" & gid_b & "' where gid='" & gid_a & "'"
        gcon.Execute(ssql)
    ...
    Private Sub btnLocate_Click(sender As Object, e As EventArgs) Handles btnLocate.Click
        ...
        For r_a = dgvJournal_a.Rows.Count - 1 To 0 Step -1
            ...
            If chqNeftNo_a = txtChequeToFind.Text Then
                ...
                tmpRS = gcon.Execute("select InstrumentBank from tblreceipt_cult where gid='" & gid_a & "'")
